  ...
  <img src="logo-name.png" alt="Logo Text" class="h-10 w-auto object-contain" />
        ...
        <div class="main w-full bg-[#FFF6E8] md:ml-[320px]">
            ...
            <main class="px-8 py-8">
                <?php
$conn = new mysqli('localhost', 'root', '', 'hr_1&2_learning_management_and_training_management1');
...
if (isset($_POST['add_program'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO `trainingprograms` (`ProgramName`, `ProgramDescription`, `EstimatedDuration`) VALUES (?, ?, ?)");
    $stmt->bind_param('sss', $programName, $programDescription, $estimatedDuration);
    $stmt->execute();
...
if (isset($_POST['edit_program'])) {
    ...
    $stmt = $conn->prepare("UPDATE `trainingprograms` SET `ProgramName` = ?, `ProgramDescription` = ?, `EstimatedDuration` = ? WHERE `ProgramID` = ?");
    $stmt->bind_param('sssi', $programName, $programDescription, $estimatedDuration, $programID);
    $stmt->execute();
    header('Location: training_programs.php');
}

if (isset($_GET['delete_program'])) {
    $programID = $_GET['delete_program'];

    // Check if the program is referenced in enrollments
    $checkEnrollments = $conn->prepare("SELECT COUNT(*) AS count FROM `Enrollments` WHERE `ProgramID` = ?");
    $checkEnrollments->bind_param('i', $programID);
    $checkEnrollments->execute();
    $result = $checkEnrollments->get_result();
    $row = $result->fetch_assoc();

    if ($row['count'] > 0) {
        echo "<script>alert('This program is referenced in enrollments and cannot be deleted.');</script>";
    } else {
        $stmt = $conn->prepare("DELETE FROM `trainingprograms` WHERE `ProgramID` = ?");
...
$programs = $conn->query("SELECT * FROM `trainingprograms`");
?>
...
<body class="bg-white text-[#4E3B2A] min-h-screen">
    <div class="main-content px-4 md:px-10 py-8">
        <h1 class="text-3xl font-bold mb-10 text-center">Training Programs</h1>
        <div class="text-left mb-6">
            <button onclick="openModal('addModal')" class="bg-[#F7E6CA] text-[#4E3B2A] border-2 border-[#594423] rounded-lg px-6 py-3 font-semibold hover:bg-[#594423] hover:text-white transition"><i class='bx bx-plus'></i> Add Program</button>
        </div>
            ...
            <table class="w-full border-separate border-spacing-y-3">
                <thead>
                    <tr class="bg-[#F7E6CA]">
                        ...
                        <th class="py-4 px-6 text-left text-base font-semibold uppercase border-2 border-[#594423]">Program Name</th>
                        <th class="py-4 px-6 text-left text-base font-semibold uppercase border-2 border-[#594423]">Program Description</th>
                        <th class="py-4 px-6 text-left text-base font-semibold uppercase border-2 border-[#594423]">Duration</th>
                ...
                <tbody>
                    <?php while ($row = $programs->fetch_assoc()): ?>
                        <tr class="bg-[#FFF6E8] hover:bg-[#F7E6CA] transition">
                            <td class="py-4 px-6 rounded-l-lg"><?= $row['ProgramID'] ?></td>
                            <td class="py-4 px-6"><?= $row['ProgramName'] ?></td>
                            <td class="py-4 px-6"><?= $row['ProgramDescription'] ?></td>
                            <td class="py-4 px-6"><?= $row['EstimatedDuration'] ?></td>
                            <td class="py-4 px-6 flex gap-2 rounded-r-lg">
                                <button onclick="openEditModal(<?= $row['ProgramID'] ?>, '<?= $row['ProgramName'] ?>', '<?= $row['ProgramDescription'] ?>', '<?= $row['EstimatedDuration'] ?>')" class="bg-[#FFF6E8] text-[#594423] border-2 border-[#594423] rounded-md px-4 py-2 hover:bg-[#F7E6CA] transition">Edit</button>
                                <a href="?delete_program=<?= $row['ProgramID'] ?>" onclick="return confirm('Are you sure?')" class="bg-[#594423] text-white border-2 border-[#594423] rounded-md px-4 py-2 hover:bg-[#402f1c] transition">Delete</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
    ...
    <div id="addModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md mx-2">
            <div class="flex justify-between items-center border-b pb-3 mb-4 p-6">
                <h2 class="text-xl font-bold">Add Program</h2>
                <button type="button" class="text-2xl" onclick="closeModal('addModal')">&times;</button>
            </div>
            <form method="POST" class="p-6 pt-0">
                <label for="programName" class="block font-semibold mb-1">Program Name:</label>
                <input type="text" name="programName" id="programName" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]">

                <label for="programDescription" class="block font-semibold mb-1">Description:</label>
                <textarea name="programDescription" id="programDescription" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]"></textarea>

                <label for="estimatedDuration" class="block font-semibold mb-1">Estimated Duration:</label>
                <input type="text" name="estimatedDuration" id="estimatedDuration" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]">

                <div class="flex justify-end gap-2 mt-6">
                    <button type="button" class="bg-gray-200 text-[#4E3B2A] rounded px-4 py-2" onclick="closeModal('addModal')">Close</button>
                    <button type="submit" name="add_program" class="bg-[#F7E6CA] text-[#4E3B2A] border-2 border-[#594423] rounded px-4 py-2 hover:bg-[#594423] hover:text-white transition">Add Program</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md mx-2">
            ...
            <form method="POST" class="p-6 pt-0">
                <input type="hidden" name="programID" id="edit_programID">

                <label for="edit_programName" class="block font-semibold mb-1">Program Name:</label>
                <input type="text" name="programName" id="edit_programName" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]">

                <label for="edit_programDescription" class="block font-semibold mb-1">Description:</label>
                <textarea name="programDescription" id="edit_programDescription" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]"></textarea>

                <label for="edit_estimatedDuration" class="block font-semibold mb-1">Estimated Duration:</label>
                <input type="text" name="estimatedDuration" id="edit_estimatedDuration" required class="w-full border-2 border-[#594423] rounded px-3 py-2 mb-4 focus:outline-none focus:border-[#4E3B2A]">

                <div class="flex justify-end gap-2 mt-6">
                    <button type="button" class="bg-gray-200 text-[#4E3B2A] rounded px-4 py-2" onclick="closeModal('editModal')">Close</button>
                    <button type="submit" name="edit_program" class="bg-[#FFF6E8] text-[#594423] border-2 border-[#594423] rounded px-4 py-2 hover:bg-[#F7E6CA] transition">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(id, name, description, duration) {
            document.getElementById('edit_programID').value = id;
            document.getElementById('edit_programName').value = name;
            document.getElementById('edit_programDescription').value = description;
            document.getElementById('edit_estimatedDuration').value = duration;
            openModal('editModal');
        }
    </script>
</body>
